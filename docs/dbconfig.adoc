...
automua uses the SQLAlchemy toolkit to access databases.
...
If you upgrade from an early automua release and wish to migrate your existing database, you can use the built-in Alembic support.
...
This section demonstrates what you need to do to in order to use SQLite version 3 or higher as a backend database for automua.

Step 1: Set the database URI in your automua configuration.
...
Starting with automua version 2022.0, JSON data can be used to populate the database in a simplified manner, without the need to use SQL statements.
...
Make sure to match the automua.conf `db_uri` setting when specifying the database.
...
Step 2: Set the database URI in your automua configuration.
The following example uses _pymysql_ as a DB driver, which is not included in the automua distribution.
...
Step 2: Set the database URI in your automua configuration.
The following example uses _psycopg2_ as a DB driver, which is not included in the automua distribution.
